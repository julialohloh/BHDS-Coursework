...
Start from i = 1 since DNA seq counts starts from 1 and then increment by 1 
in i +=1 """
...
"""Set multiplier factor which is score 1 divided by no. of sequences.
list is set at 0.25 (score 1 / 4 sequences) and then * 5 (len of 
...
factor = float(1/float(no_of_seq))
multipliers = [factor]*(int(no_of_seq))
weighted = [[multipliers[i] * j for j in x]
      for i, x in enumerate(tabulated)]
print("The weighted table is : " + str(weighted))

"""use in-built max function to get the max index in weighted table. Weighted table is zip to return iterables as tuples. Consensus/base score is the sum of all the max index. use a for loop to check whether there is an element with 0.25 because its means for that column each base has the same count and remove it if there is, else print consensus"""
...
for values in total_list:
    match = (float(values)/ float(base_score))*100
...
for base in new_dna_list[0][4]:
    ...
    match_new_seq = (float(Total_score_new_seq)/ float(base_score))*100
