...
for index, x in enumerate(total_list):
    if round(((x/3.5)*100),2) >= 70.0:
        print("The sequence weight for bases in sequence 3 that falls within 70% threshold is " + str(x) +  " and corresponding location is : " + str(index+1))
    else:
        continue
...
for i, x in enumerate(thresholdlist):
    ...
    if x <=70.0:
        ...
        print("The location of match found in Sequence 3 is " + str(i+1) + " with threshold of " + str(x))
        continue
...
for i in range(i,len(thresholdlist),5):
    ...
    print(newlist)
    print("Consolidated value of " + str(max(newlist)) + " for Position " + str(i+1) + " to Position " + str(i+5) + " in Sequence 3 ")
